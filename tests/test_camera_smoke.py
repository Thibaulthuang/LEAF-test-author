...
class CameraSmokeTests(unittest.TestCase):
    ...
    def test_run_camera_direct_smoke_launches_builtin_camera_and_collects_evidence(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["evidence"]["ui_snapshot_refs"][0]["phase"], "after_launch")
            self.assertEqual(result["evidence"]["ui_snapshot_refs"][0]["action_id"], "camera_direct")
            self.assertTrue((root / result["evidence"]["ui_snapshots"]["after_launch"]["index_path"]).is_file())
            self.assertTrue((root / result["evidence"]["ui_snapshot_refs"][0]["raw_path"]).is_file())
            self.assertTrue((root / result["evidence"]["ui_snapshot_refs"][0]["index_path"]).is_file())
    ...
    def test_run_camera_capture_e2e_clicks_verified_shutter_node(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual([ref["phase"] for ref in result["evidence"]["ui_snapshot_refs"]], ["before_capture", "after"])
            ...
            self.assertTrue((root / result["evidence"]["ui_snapshot_refs"][0]["raw_path"]).is_file())
            self.assertTrue((root / result["evidence"]["ui_snapshot_refs"][1]["index_path"]).is_file())
