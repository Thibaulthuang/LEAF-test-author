...
class LeafAuthorStageTests(unittest.TestCase):
    ...
    def test_advance_camera_direct_real_smoke_records_real_device_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(preflight["decision_contract"]["agent_mode"], "orchestrator")
            self.assertEqual(preflight["decision_contract"]["handoff_required"], False)
            self.assertEqual(preflight["decision_contract"]["required_inputs"], ["run_id", "workflow", "decision_contract"])
            self.assertEqual(preflight["decision_contract"]["subagent_boundary"], "workflow_orchestration")
    ...
    def test_advance_camera_capture_real_e2e_requires_approval_token_before_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(approval["decision_contract"]["agent_owner"], "leaf-test-author")
            self.assertEqual(approval["decision_contract"]["agent_mode"], "orchestrator")
            self.assertEqual(approval["decision_contract"]["handoff_required"], False)
            self.assertEqual(approval["decision_contract"]["context_slice"], ["workflow", "real_device_approval"])
            self.assertEqual(approval["user_loop"]["position"], "approve_real_device")
            self.assertEqual(approval["user_loop"]["required_input"], "approve_camera_capture_e2e")
    ...
    def test_advance_real_runtime_requires_serial_before_local_or_device_stages(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(input_payload["decision_contract"]["agent_owner"], "leaf-test-author")
            self.assertEqual(input_payload["decision_contract"]["agent_mode"], "orchestrator")
            self.assertEqual(input_payload["decision_contract"]["handoff_required"], False)
            self.assertEqual(input_payload["decision_contract"]["context_slice"], ["workflow", "real_device_input"])
            self.assertEqual(input_payload["user_loop"]["position"], "provide_target_inputs")
            self.assertEqual(input_payload["user_loop"]["required_input"], "--serial <serial>")
