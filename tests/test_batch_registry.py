...
class BatchRegistryTests(unittest.TestCase):
    ...
    def test_resume_batch_auto_safe_blocks_when_batch_audit_failed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["action_route"]["next_action"], "inspect_batch_audit")
            self.assertEqual(result["action_route"]["command"], "python3 -m tools.leaf_author report-batch <batch_id>")
            ...
            self.assertEqual(result["action_route"]["subagent_boundary"], "batch_audit_triage")
            self.assertEqual(result["action_route"]["context_slice"], ["batch_audit_summary", "focus_plan"])
            ...
            self.assertEqual(result["user_loop"]["required_input"], "inspect batch_audit_summary.failed_checks")
