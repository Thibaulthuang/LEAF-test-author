...
class OpenCodeWorkflowDocsTests(unittest.TestCase):
    ...
    def test_leaf_report_documents_operator_decision_entrypoint(self):
        ...
        self.assertIn("report-batch", command)
        self.assertIn("inspect-ui-tree", command)
        ...
        self.assertIn("action_route", command)
        ...
        self.assertIn("runtime_evidence_summary.ui_snapshots", command)
    ...
    def test_workflow_contract_documents_phase_quality_gate_and_user_boundaries(self):
        ...
        self.assertEqual(contract["context_policy"]["ui_tree_inspection"], "runtime_evidence_summary_then_selected_indexes")
        ...
        self.assertIn("inspect_ui_tree", contract["entrypoints"])
    ...
    def test_readme_documents_multi_run_context_management(self):
        ...
        self.assertIn("runtime-evidence-contract camera", readme)
        self.assertIn("real-device gate status", readme)
        self.assertIn("runtime registry status", readme)
        self.assertIn("opencode-contract", readme)
        self.assertIn("phase-guard", readme)
        self.assertIn("target-policy", readme)
        self.assertIn("agent-handoff-contract", readme)
        self.assertIn("agent_mode", readme)
        self.assertIn("handoff_required", readme)
        ...
        self.assertIn("target_policy", readme)
        self.assertIn("matches that run's resume", readme)
        self.assertIn("subagent_boundary", readme)
        self.assertIn("required_inputs", readme)
    ...
    def test_opencode_docs_do_not_suggest_test_hap_for_camera_main_path(self):
        ...
        self.assertIn("--runtime-mode capture_e2e", combined)
        self.assertNotIn("--camera-direct", combined)
        self.assertNotIn("--camera-capture", combined)
    ...
    def test_leaf_author_documents_stable_triggers_context_agents_and_user_loop(self):
        ...
        self.assertIn("agent_mode", skill)
        self.assertIn("handoff_required", skill)
        self.assertIn("required_inputs", skill)
        self.assertIn("subagent_boundary", skill)
        self.assertIn("specific_question", skill)
        self.assertIn("requires_user_confirmation", skill)
        self.assertIn("safe_to_auto_continue", skill)
        self.assertIn("opencode-contract", skill)
        self.assertIn("phase-guard", skill)
        self.assertIn("target-policy", skill)
        self.assertIn("agent-handoff-contract", skill)
        self.assertIn("action_routes", skill)
        self.assertIn("command", skill)
        self.assertIn("operator command", skill)
        self.assertIn("do not dispatch from memory", skill)
        self.assertIn("real-device-contract", skill)
        self.assertIn("runtime-evidence-contract", skill)
